...
│   ├── 09_far_sensitivity.py           # FAR sensitivity analysis for Dual-Trigger MSR Protocol (K ∈ {3,5,7})
...
│   ├── far_sensitivity_K357.csv               # FAR estimates for K ∈ {3,5,7}, H ∈ {1,5,22} (Appendix D, Table D1)
│   └── far_sensitivity_K357.txt               # Human-readable FAR sensitivity report
│
...
### v2 (2026-04-30)
- Added `scripts/09_far_sensitivity.py`: FAR sensitivity analysis for the Dual-Trigger Dynamic MSR Protocol across K ∈ {3, 5, 7} consecutive-day thresholds
- Added `results/far_sensitivity_K357.csv` and `results/far_sensitivity_K357.txt`: pre-filter FAR estimates (K=3: ~12.0, K=5: ~4.9, K=7: ~1.0 per year at H=5)
- Updated manuscript Section 6.2 FAR estimates to reflect backtesting output
