...
- **DOI:** https://doi.org/10.5281/zenodo.19913059

### Added
- `09_far_sensitivity.py` — FAR sensitivity analysis for Dual-Trigger Dynamic MSR Protocol; estimates pre-filter false alarm rates across K ∈ {3, 5, 7} consecutive-day thresholds using Bernoulli sliding-window model on walk-forward backtesting data
- `results/far_sensitivity_K357.csv` — FAR estimates for all K and H combinations (H ∈ {1, 5, 22})
- `results/far_sensitivity_K357.txt` — Human-readable FAR sensitivity report

### Changed
- Manuscript Section 6.2: updated pre-filter FAR estimates to reflect `09_far_sensitivity.py` output (K=3: ~12.0/year, K=5: ~4.9/year, K=7: ~1.0/year at H=5); added reference to Appendix D, Table D1 Phase 1 KPI
