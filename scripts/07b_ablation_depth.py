# =============================================================================
# 07b_ablation_depth_fixed.py  —  v2
# =============================================================================
# BUG 1 [CRITICAL] — PennyLane serial → numpy FastQuantumKernel (~40x faster)
# BUG 2 — FEATURES thiếu CPI_return, PHASE_dummy
# BUG 3 — svr_params từ rbf_svm optuna → fixed C=1.0, eps=0.1 (ablation protocol)
...
# =============================================================================
FAST_MODE = False   # True = 5 folds, 3 seeds (~5 phút) | False = full (~3h)
...
CHECKPOINT  = OUTPUT_DIR / "ablation_ab2_checkpoint.parquet"
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

TARGET   = "EUA_return"
FEATURES = [                           # BUG 2 FIX: 8 features
...
N_QUBITS      = 4      # fixed
NYSTROM_M     = 100
SVR_C         = 1.0    # BUG 3 FIX: fixed params cho ablation
SVR_EPSILON   = 0.1
DEPTH_CONFIGS = [1, 2, 3]
...
if FAST_MODE:
    ...
    N_SEEDS   = 10; MAX_FOLDS = None   # 10 seeds đủ cho ablation
    logging.info(f"FULL MODE: all folds, {N_SEEDS} seeds, depths={DEPTH_CONFIGS}")


# ── FastQuantumKernel (BUG 1 FIX) ────────────────────────────────
class FastQuantumKernel:
    def __init__(self, n_qubits, n_layers, seed):
    ...
    def compute_states(self, X):
        N=len(X); Xc=np.clip(X[:,:self.n_qubits],-np.pi,np.pi)
        s=np.zeros((N,self.dim),dtype=complex); s[:,0]=1.0
        for l in range(self.n_layers):
            for q in range(self.n_qubits): s=self._rx_batch(s,Xc[:,q],q)
            s = s@self._cnot.T @ self._ry[l].T
        return s
...
def run_qksvr(X_tr, y_tr, X_te, n_qubits, n_layers, seed):
    ...
    Kll_inv = np.linalg.pinv(v@np.diag(w)@v.T)
    Ktrlm   = fqk.kernel(Xtr,Xlm)
    Knys    = Ktrlm@Kll_inv@Ktrlm.T
    ...
    return svr.predict(Ktelm@Kll_inv@Ktrlm.T)
...
def main():
    ...
    folds = load_folds(FOLDS_PATH)   # BUG 5 FIX
    ...
    if CHECKPOINT.exists():
        ...
        done_set = set(zip(df_done["n_layers"],df_done["fold_id"],df_done["horizon"]))
        results  = df_done.to_dict("records")
        logging.info(f"Resume: {len(done_set)} done")
    else:
        done_set, results = set(), []

    total = len(DEPTH_CONFIGS)*len(folds); counter = 0
    t0_all = time.time()
    logging.info(f"AB2 Depth | configs={DEPTH_CONFIGS} | seeds={N_SEEDS} | folds/config={len(folds)}")

    for n_layers in DEPTH_CONFIGS:
        logging.info(f"\n{'='*50}\n  n_layers={n_layers}\n{'='*50}")
        for _, fold in folds.iterrows():
            fid=int(fold["fold_id"]); h=int(fold["horizon"]); counter+=1
            if (n_layers,fid,h) in done_set: continue
            ...
            for seed in range(N_SEEDS):
                try: preds.append(run_qksvr(X_tr,y_tr,X_te,N_QUBITS,n_layers,seed))
            ...
            eta_sec   = (total-counter)/max(counter/max(time.time()-t0_all,1e-6),1e-6)
            eta_str   = f"{eta_sec/3600:.1f}h" if eta_sec>3600 else f"{eta_sec/60:.0f}m"

            results.append({
                "ablation": "AB2_depth", "config": f"n_layers={n_layers}",
                "n_qubits": N_QUBITS,   "n_layers": n_layers,
                "fold_id":  fid,        "horizon": h,
                "y_true":   float(y_te[0]),    # scalar
                "y_pred":   float(y_pred[0]),  # scalar
                "fold_rmse": fold_rmse, "n_seeds": len(preds), "time_sec": elapsed,
            })
            done_set.add((n_layers,fid,h))
            logging.info(f"  [{counter}/{total}] layers={n_layers} Fold {fid:03d} H={h}d | "
                         f"RMSE={fold_rmse:.5f} | {elapsed:.1f}s | ETA {eta_str}")
            if len(results)%10==0: save_ckpt(results)

    df_out = pd.DataFrame(results)
    out    = OUTPUT_DIR / "ablation_ab2_depth.parquet"
    ...
    summary.to_csv(OUTPUT_DIR/"ablation_ab2_depth_summary.csv", index=False)

    total_min = (time.time()-t0_all)/60
    print(f"\n{'='*50}\n  AB2 DEPTH ABLATION SUMMARY\n{'='*50}")
